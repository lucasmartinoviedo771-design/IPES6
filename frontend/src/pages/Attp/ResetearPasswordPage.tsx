...
export default function ResetearPasswordPage() {
  ...
  const resetMutation = useMutation({
    ...
    onSuccess: (_data, dni) => {
      toast.success(`Contraseña reseteada. El estudiante puede ingresar con: pass${dni}`);
    ...
    <Box p={2} display="flex" flexDirection="column" gap={3} maxWidth={700} mx="auto">
      ...
      <Box>
        ...
        <Typography variant="body2" color="text.secondary">
          Buscá al estudiante por DNI o apellido y restablecé su contraseña. La nueva clave será <strong>pass + DNI</strong>.
        </Typography>
      </Box>
                ...
                <TableRow key={e.dni} hover>
                  ...
                  <TableCell align="center">
                    <Button
                      ...
                      onClick={() => setSelectedEstudiante({ dni: e.dni, nombre: e.nombre, apellido: e.apellido })}
                    >
                      Resetear
                    </Button>
                  </TableCell>
      ...
      <Dialog open={!!selectedEstudiante} onClose={() => setSelectedEstudiante(null)} maxWidth="xs" fullWidth>
        <DialogTitle>Confirmar reset de contraseña</DialogTitle>
        <DialogContent>
          <Typography>
            ¿Resetear la contraseña de <strong>{selectedEstudiante?.apellido}, {selectedEstudiante?.nombre}</strong>?
          </Typography>
          <Typography variant="body2" color="text.secondary" mt={1}>
            La nueva contraseña será: <strong>pass{selectedEstudiante?.dni}</strong>
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setSelectedEstudiante(null)} disabled={resetMutation.isPending}>
            Cancelar
          </Button>
          <Button
            variant="contained"
            color="warning"
            onClick={() => selectedEstudiante && resetMutation.mutate(selectedEstudiante.dni)}
            disabled={resetMutation.isPending}
            startIcon={resetMutation.isPending ? <CircularProgress size={16} color="inherit" /> : <LockResetIcon />}
          >

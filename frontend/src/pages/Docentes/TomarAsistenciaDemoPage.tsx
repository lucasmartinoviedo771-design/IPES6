...
export default function TomarAsistenciaDemoPage() {
	...
	const fechaLegible = dayjs(MOCK_CLASE.fecha)
		.locale("es")
	...
	const handleMarcarDocente = () => {
		...
		setTimeout(() => {
			...
			setIsSaving(false);
			enqueueSnackbar("Tu asistencia ha sido registrada (DEMO).", {
				variant: "success",
			});
		}, 1000);
	};
	...
	const handleGuardarEstudiantes = () => {
		...
		setTimeout(() => {
			setIsSaving(false);
			setHasChanges(false);
			enqueueSnackbar("Asistencia de estudiantes guardada (DEMO).", {
				variant: "success",
			});
		}, 1000);
	};

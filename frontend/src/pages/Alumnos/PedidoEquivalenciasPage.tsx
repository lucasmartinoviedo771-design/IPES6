...
const PedidoEquivalenciasPage: React.FC = () => {
  ...
  const [carrerasAlumno, setCarrerasAlumno] = useState<TrayectoriaCarreraDetalleDTO[]>([]);
  ...
  const carreraOrigenSeleccionada = useMemo(
    () => carrerasAlumno.find((c) => String(c.profesorado_id) === form.profesoradoOrigenId),
    [carrerasAlumno, form.profesoradoOrigenId],
  );
  ...
  useEffect(() => {
    ...
    obtenerTrayectoriaAlumno(canGestionar ? { dni: dniObjetivo || undefined } : undefined)
      .then((data) => {
        if (!cancelado) {
          setTrayectoria(data);
        }
  ...
  const handleDescargar = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `pedido_equivalencias_${dniObjetivo || user?.dni || "alumno"}.pdf`;
      link.click();
                      ...
                      <>
                        <Grid item xs={12} md={6}>
                          <FormControl
                            fullWidth
                            size="small"
                            disabled={datosDeshabilitados || !puedeEditar || carrerasLoading || carrerasAlumno.length === 0}
                          >
                            ...
                            <Select
                              ...
                              onChange={(event) => {
                                const value = String(event.target.value);
                                const carrera = carrerasAlumno.find((item) => String(item.profesorado_id) === value);
                                setForm((prev) => ({
                                  ...prev,

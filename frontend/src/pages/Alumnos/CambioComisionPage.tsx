...
const defaultHistorial: HistorialAlumnoDTO = {
  aprobadas: [],
  regularizadas: [],
  inscriptas_actuales: [],
};
...
const CambioComisionPage: React.FC = () => {
  ...
  const isAlumno = !canGestionar;
  const shouldFetch = isAlumno || normalizedDni.length > 0;

  const materiasQ = useQuery({
    queryKey: ['cc-materias', normalizedDni],
    queryFn: async () => (await obtenerMateriasPlanAlumno(normalizedDni ? { dni: normalizedDni } : undefined)).map(mapMateria),
    enabled: shouldFetch,
  });

  const historialQ = useQuery({
    queryKey: ['cc-historial', normalizedDni],
    queryFn: () => obtenerHistorialAlumno(normalizedDni ? { dni: normalizedDni } : undefined),
    enabled: shouldFetch,
  });

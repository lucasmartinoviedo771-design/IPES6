...
import { buscarEstudiantesGlobal, agregarCarreraEstudiante, EstudianteGlobalResultDTO } from "@/api/estudiantes/admin";
import { listarTodosProfesorados, ProfesoradoDTO } from "@/api/cargaNotas";
...
export function AgregarEstudianteExternoPanel() {
  ...
  const profesoradosDisponibles: ProfesoradoDTO[] = myProfIds !== null
    ? profesorados.filter((p) => myProfIds.includes(p.id))
    : profesorados;

  const agregarMutation = useMutation({
    mutationFn: ({ dni, profesorado_id }: { dni: string; profesorado_id: number }) =>
      agregarCarreraEstudiante(dni, { profesorado_id }),
    onSuccess: (_data, vars) => {
      enqueueSnackbar("Estudiante agregado a la carrera correctamente", { variant: "success" });
  ...
  const profDestino = profesoradoDestino !== "" ? profesoradoDestino : profesoradosDisponibles[0]?.id;
      ...
      <FormControlLabel
        ...
        label={
          <Stack direction="row" alignItems="center" gap={0.5}>
            <PersonSearchIcon fontSize="small" color={activo ? "primary" : "disabled"} />
            <Typography variant="body2" color={activo ? "primary" : "text.secondary"}>
                    ...
                    return (
                      <Box
                        ...
                        sx={{
                          ...
                          border: "1px solid",
                          borderColor: "divider",
                          bgcolor: enDestino ? "action.hover" : "background.paper",
                        }}
                      >
                        <Box>
                          ...
                          <Stack direction="row" gap={0.5} flexWrap="wrap" mt={0.5}>
                            ...
                            {est.carreras.length === 0 && (
                              <Typography variant="caption" color="text.secondary">Sin carreras</Typography>
                            )}
                          </Stack>
                        </Box>

                        {enDestino ? (
                          <Chip label="Ya está en esta carrera" size="small" color="default" />
                        ) : (
                          <Button
                            size="small"
                            variant="contained"
                            ...
                            onClick={() => profDestino && agregarMutation.mutate({ dni: est.dni, profesorado_id: profDestino })}
                          >
                            Agregar
                          </Button>
                        )}

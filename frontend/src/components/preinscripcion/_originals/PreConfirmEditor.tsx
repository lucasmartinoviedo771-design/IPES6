...
import {
  ...
  listarPreinscripcionesEstudiante, agregarCarreraPreinscripcion, PreDocItem,
} from "@/api/preinscripciones";
import { useAuth } from "@/context/AuthContext";
import { fetchCarreras, type Carrera } from "@/api/carreras";
...
function getExtra(extra: Record<string, unknown>, key: string): unknown {
  return extra[key];
}
          ...
          <Stack spacing={2} sx={{ position: 'sticky', top: 16 }}>
            ...
            <Paper variant="outlined" sx={{ p: 2, border: '1px solid #eee' }}>
              ...
              <Stack direction="row" spacing={1} mb={1}>
                <input id="foto-input" type="file" accept="image/*" style={{ display: 'none' }} onChange={(e: React.ChangeEvent<HTMLInputElement>) => { const f = e.target.files?.[0]; if (f) compressImage(f).then(({ file }) => mUploadFoto.mutate(file)).catch(() => enqueueSnackbar("No se pudo procesar la imagen", { variant: "error" })); }} />
                <label htmlFor="foto-input"><Button size="small" variant="outlined" component="span">Cargar archivo</Button></label>
              </Stack>
              {(() => {
                const docs: PreDocItem[] = docsQ.data ?? [];

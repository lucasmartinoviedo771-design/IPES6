...
export default function FotoPerfilEditor({ fotoUrl, nombre }: Props) {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      enqueueSnackbar("No se pudo acceder a la cámara. Revisá los permisos del navegador.", { variant: "warning" });
      setMode("menu");
    }
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_SIZE_MB * 1024 * 1024) {
      enqueueSnackbar(`La imagen no puede superar ${MAX_SIZE_MB}MB.`, { variant: "warning" });
      return;
    }
  ...
  const mutation = useMutation({
    ...
    onError: () => {
      enqueueSnackbar("No se pudo guardar la foto. Intentá nuevamente.", { variant: "error" });
    },
  });
  ...
  return (
    <>
      {/* Avatar clickeable */}
      <Box sx={{ position: "relative", width: 100, height: 100, cursor: "pointer" }} onClick={() => setOpen(true)}>
        <Avatar
          src={fotoUrl ?? undefined}
          sx={{ width: 100, height: 100, fontSize: 36, bgcolor: "primary.light" }}
        >
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="xs" fullWidth>
          ...
          {mode === "camera" && !preview && (
            <Box display="flex" flexDirection="column" alignItems="center" gap={2}>
              <Box
                ...
                playsInline
                muted
                sx={{ width: "100%", maxHeight: 280, borderRadius: 2, bgcolor: "black" }}
              />
              <Button variant="contained" startIcon={<PhotoCameraIcon />} onClick={captureFromCamera}>
                Capturar
              </Button>
            </Box>
          )}
          ...
          {mode === "menu" && !preview && (
            <Stack spacing={2} mt={1}>
              ...
              {isMobile ? (
                ...
                <Button variant="outlined" startIcon={<PhotoCameraIcon />} onClick={startCamera} fullWidth>
                  Usar cámara web
                </Button>
              ) : null}
        ...
        <DialogActions>
          {preview && (
            <>
              <Button onClick={() => { setPreview(null); setCapturedFile(null); setMode("menu"); }}>
                Volver
              </Button>
              <Button
                variant="contained"

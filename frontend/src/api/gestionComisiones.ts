...
export const listarComisionesGestion = async (materiaId: number, anioLectivo: number): Promise<ComisionGestionDTO[]> => {
    const response = await api.get(`/estudiantes/comisiones/materia/${materiaId}/anio/${anioLectivo}`);
...
export const distribuirEstudiantes = async (comisionOrigenId: number, comisionDestinoId: number, porcentaje: number) => {

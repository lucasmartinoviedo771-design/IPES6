...
from apps.preinscriptions.schemas import ChecklistIn, PreinscripcionIn, EstudianteIn
...
def test_crear_preinscripcion_exitoso(settings):
    ...
    pre = Preinscripcion.objects.get(carrera=carrera, estudiante__dni="12345678")
    assert resp.ok is True
    assert pre.estado == "Enviada"
    assert pre.codigo.startswith(f"PRE-{date.today().year}")
    assert pre.estudiante.user.username == "12345678"
...
def test_confirmar_por_codigo_actualiza_estado_y_flags(django_user_model):
    ...
    estudiante_user = User.objects.create_user(username="12312312")
    estudiante = Estudiante.objects.create(user=estudiante_user, dni="12312312")
    carrera = Profesorado.objects.create(
    ...
    assert estudiante_user.check_password(f"Pass{estudiante.dni}")
...
def test_observar_y_rechazar_cambian_estado(django_user_model):
    ...
    estudiante_user = User.objects.create_user(username="9999")
    estudiante = Estudiante.objects.create(user=estudiante_user, dni="9999")
    carrera = Profesorado.objects.create(nombre="Prof Obs", duracion_anios=4, activo=True, inscripcion_abierta=True)
    pre = Preinscripcion.objects.create(estudiante=estudiante, carrera=carrera, anio=date.today().year, estado="Enviada", activa=True)
    pre.codigo = _generar_codigo(pre.id)
    pre.save(update_fields=["codigo"])
...
def test_eliminar_y_activar_togglea_activa_y_estado(django_user_model):
    ...
    request = SimpleNamespace(user=user)
    estudiante_user = User.objects.create_user(username="8888")
    estudiante = Estudiante.objects.create(user=estudiante_user, dni="8888")
...
def test_agregar_carrera_crea_nueva_preinscripcion_y_bloquea_duplicado(django_user_model):
    ...
    request = SimpleNamespace(user=user)
    estudiante_user = User.objects.create_user(username="7777")
    estudiante = Estudiante.objects.create(user=estudiante_user, dni="7777")
    ...
    pre = Preinscripcion.objects.create(estudiante=estudiante, carrera=carrera_actual, anio=date.today().year, estado="Enviada", activa=True)
    pre.codigo = _generar_codigo(pre.id)
    pre.save(update_fields=["codigo"])

    resp = agregar_carrera(request, pre.codigo, SimpleNamespace(carrera_id=carrera_nueva.id, anio=None))
    assert resp.ok is True
    creada = Preinscripcion.objects.get(carrera=carrera_nueva, estudiante=estudiante)
...
@pytest.mark.django_db
def test_listar_por_estudiante_devuelve_historico_ordered(django_user_model):
    user = django_user_model.objects.create_user(username="staff5", password="admin123", is_staff=True)
    request = SimpleNamespace(user=user)
    estudiante_user = User.objects.create_user(username="6666")
    estudiante = Estudiante.objects.create(user=estudiante_user, dni="6666")
    carrera = Profesorado.objects.create(nombre="Prof Hist", duracion_anios=4, activo=True, inscripcion_abierta=True)
    pre1 = Preinscripcion.objects.create(estudiante=estudiante, carrera=carrera, anio=2024, estado="Enviada", activa=True)
    pre2 = Preinscripcion.objects.create(estudiante=estudiante, carrera=carrera, anio=2025, estado="Confirmada", activa=True)
    ...
    listado = listar_por_estudiante(request, estudiante.dni)

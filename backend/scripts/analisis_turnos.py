    ...
    if turno_actual != valid_turno:
        ...
        if count_actual > 0:
            ...
            c_correcta = Comision.objects.filter(
                materia=c.materia,
                turno__nombre__icontains=valid_turno.replace('turno ', '')
            ).first()
...
with open('/tmp/reporte_turnos.md', 'w', encoding='utf-8') as f:
    ...
    f.write("| Profesorado | Año | Materia | Turno Incorrecto (Alumnos) | Turno Correcto Ideal | ¿Existe en BD? | Alumnos en T. Correcto |\\n")
    f.write("|---|---|---|---|---|---|---|\\n")
    for r in resultados:
        ...
        f.write(f"| {prof_corto} | {r['anio']} | {r['materia']} | **{r['turno_incorrecto']} ({r['count_incorrecto']})** | {r['turno_correcto']} | {existe} | **{r['count_correcto']}** |\\n")

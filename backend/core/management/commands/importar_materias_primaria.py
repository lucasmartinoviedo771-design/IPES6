...
from backend.core.models import Materia, PlanDeEstudio, Profesorado
from django.core.management.base import BaseCommand


class Command(BaseCommand):
    help = "Importa la estructura de materias desde la carpeta Primariatemporal/2025"
    ...
    def handle(self, *args, **options):
        ...
        # Datos del Plan (Puedes cambiarlos si tienes datos reales)
        # Usaremos una resolución genérica única por ahora si no existe
        PLAN_RESOLUCION = "RES-PEP-UNICO"
        ANIO_INICIO_PLAN = 2009  # Año típico de planes vigentes, ajustable

        # Crear o buscar Profesorado
        profesorado, _ = Profesorado.objects.get_or_create(
        ...
        self.stdout.write(self.style.SUCCESS(f"Profesorado: {profesorado}"))
        ...
        if not plan:
            plan = PlanDeEstudio.objects.create(
                profesorado=profesorado, resolucion=PLAN_RESOLUCION, anio_inicio=ANIO_INICIO_PLAN, vigente=True
            )
            self.stdout.write(self.style.SUCCESS(f"Plan creado: {plan}"))
        else:
            self.stdout.write(self.style.WARNING(f"Usando plan existente: {plan}"))

        # 2. Recorrer directorios
        # Estructura esperada:
        # 2025/1º/1º Cuatrimestre/*.pdf
        # 2025/1º/2º Cuatrimestre y Anuales/*.pdf

        anios_dirs = {"1º": 1, "2º": 2, "3º": 3, "4º": 4}

        for anio_name, anio_num in anios_dirs.items():
            anio_path = os.path.join(base_path, anio_name)
            if not os.path.exists(anio_path):
                self.stdout.write(self.style.WARNING(f"No encontrado: {anio_path}"))
                continue

            self.stdout.write(f"Procesando Año: {anio_num} ({anio_name})...")
            ...
            for periodo_name in os.listdir(anio_path):
                ...
                if "1º Cuatrimestre" in periodo_name or "1° Cuatrimestre" in periodo_name:
                    ...
                    # Ojo: la carpeta se llama "2º Cuatrimestre y Anuales".
                    # Tendremos que discriminar por nombre de archivo o default a ANUAL si hay duda.
                    # Por simplicidad inicial: Si dice "Cuatrimestre", asumimos 2do, pero el nombre es ambiguo.
                    # Analizando archivos en 2023, hay muchos ANUAL.
                    # Asumiremos ANUAL por defecto salvo que el nombre del PDF diga lo contrario o sea obvio.
                ...
                for filename in os.listdir(periodo_path):
                    ...
                    nombre_limpio = self.limpiar_nombre(filename)
                    ...
                    if "taller" in nombre_lower:
                        ...
                        formato = Materia.FormatoMateria.ASIGNATURA  # No hay ateneo en modelo?
                    elif "modulo" in nombre_lower or "módulo" in nombre_lower:
                        formato = Materia.FormatoMateria.MODULO

                    # Override especial para régimen si estamos en la carpeta mixta
                    # Si el archivo viene de "2º Cuatrimestre y Anuales", es difícil saber.
                    # Sin embargo, si ya filtramos 1er cuatri, el resto cae aquí.
                    # Podemos intentar ver si el nombre dice "Cuatrimestral"
                    ...
                    self.stdout.write(f"   - [{status}] {nombre_limpio} (Año {anio_num}, {regimen})")

    def limpiar_nombre(self, filename):
        # Quitar extensión

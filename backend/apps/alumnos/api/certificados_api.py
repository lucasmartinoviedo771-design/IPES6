...
@alumnos_router.get("/certificados/alumno-regular")
def descargar_certificado_alumno_regular(
    request, 
    profesorado_id: int, 
    plan_id: int, 
    dni: str | None = None
):
    """Genera y descarga la constancia de alumno regular en formato PDF."""
    from django.db.models import Max
    from core.models import Regularidad, InscripcionMateriaAlumno
    ...
    max_anio_ins = InscripcionMateriaAlumno.objects.filter(estudiante=est, comision__materia__plan_de_estudio=plan).aggregate(Max('comision__materia__anio_cursada'))['comision__materia__anio_cursada__max']

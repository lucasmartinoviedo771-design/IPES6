...
def _user_can_manage_mesa_planilla(request, mesa: MesaExamen) -> bool:
...
def obtener_mesa_planilla(request, mesa_id: int):
    mesa = MesaExamen.objects.select_related("materia").filter(id=mesa_id).first()
    if not mesa:
    ...
    return MesaPlanillaOut(
        ...
        materia_nombre=mesa.materia.nombre if mesa.materia else "Materia",
        tipo=mesa.tipo,
...
def actualizar_mesa_planilla(request, mesa_id: int, payload: MesaPlanillaUpdateIn):
    ...
    if not _user_can_manage_mesa_planilla(request, mesa):
        return 403, ApiResponse(
            ok=False,
            message="Solo los docentes del tribunal o el personal autorizado pueden cargar notas en esta mesa.",
        )
    if not payload.alumnos:
        return 400, ApiResponse(ok=False, message="No se enviaron inscripciones.")

    valid_condiciones = {choice[0] for choice in InscripcionMesa.Condicion.choices}

    for alumno in payload.alumnos:
        inscripcion = (
            InscripcionMesa.objects.select_related("mesa")
            .filter(id=alumno.inscripcion_id, mesa_id=mesa_id)
            .first()
        )
        if not inscripcion:
            return 404, ApiResponse(ok=False, message=f"Inscripción {alumno.inscripcion_id} no encontrada.")
        if alumno.condicion and alumno.condicion not in valid_condiciones:
            return 400, ApiResponse(ok=False, message=f"Condición inválida: {alumno.condicion}")

        inscripcion.condicion = alumno.condicion or None
        inscripcion.fecha_resultado = alumno.fecha_resultado or None
        inscripcion.folio = alumno.folio or ""
        inscripcion.libro = alumno.libro or ""
        inscripcion.observaciones = alumno.observaciones or ""
        inscripcion.cuenta_para_intentos = (
            alumno.cuenta_para_intentos if alumno.cuenta_para_intentos is not None else True
        )
        inscripcion.nota = alumno.nota if alumno.nota is not None else None
        inscripcion.save(
            update_fields=[
                "condicion",
                "fecha_resultado",
    ...
    return ApiResponse(ok=True, message="Planilla guardada correctamente.")
...
def listar_constancias_examen(request, dni: str | None = None):
    ...
    for insc in inscripciones:
        if not insc.condicion:
            continue
        if insc.condicion in (InscripcionMesa.Condicion.AUSENTE, InscripcionMesa.Condicion.AUSENTE_JUSTIFICADO):
            continue
        ...
        items.append(
            ConstanciaExamenItem(
                inscripcion_id=insc.id,
                alumno=est.user.get_full_name() or est.dni,
                dni=est.dni,
                ...
                mesa_hora_hasta=str(mesa.hora_hasta)[:5] if mesa.hora_hasta else None,
    ...
    for profesorado in carreras_est:
        ...
        for plan in planes_prof:
            ...
            for materia in materias_qs:
                ...
                if reg:
                    if reg.situacion == Regularidad.Situacion.REGULAR:
                        condicion_regular = "En curso"
                        nota_reg = "-"
                    else:
                        condicion_regular = reg.get_situacion_display()
                ...
                if final_insc or acta_fila:
                    ...
                    if final_insc:
                        ...
                        if mesa_final:
                            if mesa_final.modalidad == MesaExamen.Modalidad.REGULAR:
                                final_condicion = "Regular"
                            elif mesa_final.modalidad == MesaExamen.Modalidad.LIBRE:
                                final_condicion = "Libre"
                        ...
                        if final_insc.condicion == InscripcionMesa.Condicion.APROBADO:
                            final_aprobado = True
                    ...
                    if acta_fila:
                        ...
                        permiso = (acta_fila.permiso_examen or "").upper()
                        observaciones = (acta_fila.acta.observaciones or "").lower()
                        if permiso == "EQUIV" or "equivalencia" in observaciones:
                            final_condicion = "Equivalencia"
                        elif final_condicion is None:
                            final_condicion = "Regular"
                        codigo_apr, _ = _acta_condicion(acta_fila.calificacion_definitiva)
                        if codigo_apr == "APR":
                            final_aprobado = True

                    final_evt = CartonEvento(
                        fecha=final_fecha,
                        condicion=final_condicion or "-",
                        nota=final_nota,
                        folio=final_folio,
                        libro=final_libro,

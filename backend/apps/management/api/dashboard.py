...
def global_overview(request, profesorado_id: int = None, anio: int = None):
    ...
    recientes_pre = [
        ...
        for pre in pre_qs.filter(estado="Confirmada").select_related("alumno__persona", "carrera").order_by("-updated_at")[:6]
    ]
    ...
    cambios = [
        DashboardCambioComision(
            id=c.id, estudiante=f"{c.estudiante.nombre} {c.estudiante.apellido}".strip() or c.estudiante.dni,
        ...
        for c in InscripcionMateriaEstudiante.objects.filter(comision_solicitada__isnull=False).select_related("estudiante__persona", "materia__plan_de_estudio__profesorado", "comision__turno", "comision_solicitada__turno").order_by("-updated_at")[:10]
    ]

    # 6. Analiticos
    pedidos_analiticos = [
        DashboardPedidoAnalitico(
            id=p.id, estudiante=f"{p.estudiante.nombre} {p.estudiante.apellido}".strip() or p.estudiante.dni,
        ...
        for p in PedidoAnalitico.objects.select_related("estudiante__persona", "profesorado").order_by("-created_at")[:10]
    ]
    ...
    regularidades = [
        DashboardRegularidad(
            id=reg.id, estudiante=f"{reg.estudiante.nombre} {reg.estudiante.apellido}".strip() or reg.estudiante.dni,
        ...
        for reg in Regularidad.objects.select_related("estudiante__persona", "materia__plan_de_estudio__profesorado").order_by("-fecha_cierre")[:10]
    ]

...
def listar_mesas_estudiante(
    ...
    # Si NO es personal administrativo, restringir fuertemente lo que ven
    ...
    if not es_staff:
        ...
        # Y solo mesas que pertenezcan a ventanas activas (si no se especificó una)
        if not ventana_id:
    ...
    # Pre-calcular aprobadas para filtrar si solo_rendibles
    ...
    if estudiante_para_rendir:
        ...
        # Tambien regularidades aprobadas/promocionadas
    ...
    for m in qs.order_by("fecha", "hora_desde"):
        ...
        if m.tipo in MESA_TIPOS_ORDINARIOS:
            ...
            if not legajo_ok:
                ...
                pre = (
                    (
                        Preinscripcion.objects.filter(estudiante=estudiante_para_rendir, carrera=prof)
                        .order_by("-anio", "-id")
                        .first()
                    )
                    if prof
    ...
    if mesa.modalidad == MesaExamen.Modalidad.REGULAR:
        if mesa.tipo in MESA_TIPOS_ORDINARIOS:
            ...
            if not legajo_ok:
                ...
                pre = (
                    (
                        Preinscripcion.objects.filter(estudiante=est, carrera=prof)
                        .order_by("-anio", "-id")
                        .first()
                    )
                    if prof
            ...
            if not reg or reg.situacion != Regularidad.Situacion.REGULAR:
                if reg and reg.situacion in (
                    ...
                    return 400, {"message": "Materia ya aprobada/promocionada en cursada: no requiere final."}

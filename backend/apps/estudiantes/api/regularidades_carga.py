...
def guardar_planilla_regularidad(request, payload: RegularidadCargaIn = Body(...)):
    ...
    with transaction.atomic():
        for estudiante in payload.estudiantes:
            if estudiante.situacion not in situaciones_validas:
                return 400, ApiResponse(
                    ok=False,
                    message=f"Situacion '{estudiante.situacion}' no permitida para el formato de la materia.",
                )

            situacion_codigo = ALIAS_TO_SITUACION.get(estudiante.situacion)
            if not situacion_codigo:
                return 400, ApiResponse(ok=False, message=f"Situacion desconocida: {estudiante.situacion}")

            if estudiante.situacion == "PROMOCION" and estudiante.nota_final is not None and estudiante.nota_final < 8:
                return 400, ApiResponse(
                    ok=False,
                    message="La nota final debe ser >= 8 para registrar PROMOCION.",
                )

            inscripcion_qs = InscripcionMateriaEstudiante.objects.filter(
                id=estudiante.inscripcion_id,
            )
            ...
            if not inscripcion:
                if is_virtual:
                    message = f"Inscripcion {estudiante.inscripcion_id} no corresponde a la materia sin comision."
                else:
            ...
            asistencia = estudiante.asistencia or 0
            excepcion = estudiante.excepcion or False
            ...
            Regularidad.objects.update_or_create(
                ...
                defaults={
                    ...
                    "fecha_cierre": fecha,
                    "nota_trabajos_practicos": Decimal(str(estudiante.nota_tp)) if estudiante.nota_tp is not None else None,
                    "nota_final_cursada": estudiante.nota_final,
                    "asistencia_porcentaje": estudiante.asistencia,

...
from core.models import Comision, Materia, InscripcionMateriaEstudiante, Turno, Estudiante
...
def listar_comisiones_gestion(request, materia_id: int, anio_lectivo: int):
    comisiones = Comision.objects.filter(
        ...
        cantidad_inscriptos=Count('inscripciones', filter=Q(inscripciones__estado=InscripcionMateriaEstudiante.Estado.CONFIRMADA))
    ).order_by('codigo')
...
def distribuir_estudiantes(request, payload: DistribuirEstudiantesIn):
    with transaction.atomic():
        inscripciones = list(InscripcionMateriaEstudiante.objects.filter(
...
def mover_estudiantes(request, payload: MoverEstudiantesIn):
    updated = InscripcionMateriaEstudiante.objects.filter(
